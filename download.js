...
async function triggerBackupAndWait(page, baseUrl) {
    const backupUrl = `${baseUrl}/backup-incremental.php?c`;
    ...
    while (true) { // 无限循环直到成功
        ...
        if (bodyText.includes('404') || bodyText.includes('Not Found')) {
            throw new Error('backup-incremental.php 不存在');
        }
        
        // 检查返回状态
        if (bodyText === 'success') {
            console.log(`  压缩完成（共 ${attempts} 次调用）`);
            return true;
        } else if (bodyText.startsWith('progress:')) {
            // 还在压缩中，显示进度
            const progress = bodyText.replace('progress:', '');
            console.log(`  压缩进度: ${progress} (第 ${attempts} 次)`);
            // 等待 2 秒后继续下一次
            await page.waitForTimeout(2000);
            continue;
...
async function deleteBackup(page, baseUrl) {
    const deleteUrl = `${baseUrl}/backup-incremental.php?d`;
